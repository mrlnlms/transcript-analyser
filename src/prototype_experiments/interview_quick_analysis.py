...
class QuickInterviewAnalyzer:
    def __init__(self):
        ...
        self.uncertainty_words = ['acho', 'talvez', 'sei', 'pode', 'talvez', 'quizá', 'provavelmente']
        
        # Marcadores de hesitação
        self.hesitation_markers = ['eh', 'né', 'tipo', 'assim', 'ué', 'ah', 'oh']
        
        # Palavras emocionais positivas/negativas
        self.positive_words = ['amo', 'gosto', 'bom', 'feliz', 'alegre', 'esperança', 'sonho', 'motivação']
        self.negative_words = ['triste', 'ruim', 'difícil', 'problema', 'medo', 'ansiedade', 'pressão', 'stress']
    ...
    def extract_topics_simple(self, text, n_topics=5):
        """Extração simples de tópicos para análise rápida"""
    ...
    def analyze_linguistic_patterns(self, text, topics):
        ...
        for topic in topics:
            ...
            if topic_sentences:
                ...
                topic_hesitations = sum(1 for word in topic_text.split() 
                                      if word in self.hesitation_markers)
                topic_uncertainty = sum(1 for word in topic_text.split() 
    ...
    def create_concept_network(self, text, topics):
        """Cria rede de co-ocorrência de conceitos"""
    ...
    def generate_auto_insights(self, analysis_results):
        """Gera insights automáticos baseados nos padrões detectados"""
        insights = []
    ...
    def generate_quick_report(self, text, participant_name="Participante"):
        """Gera relatório rápido e automático"""
        print("🚀 GERANDO RELATÓRIO RÁPIDO...")
        print("=" * 80)
        ...
        topics, topic_distribution = self.extract_topics_simple(clean_text)
        ...
        linguistic_patterns = self.analyze_linguistic_patterns(clean_text, topics)
        
        # 6. Rede de conceitos
        concept_network = self.create_concept_network(clean_text, topics)
        ...
        self.print_formatted_report(analysis_results)
        
        return analysis_results
    
    def print_formatted_report(self, results):
        """Imprime relatório formatado"""
        
        print(f"\n{'='*80}")
        print(f"📋 RELATÓRIO RÁPIDO DE ANÁLISE - {results['participant']}")
        print(f"{'='*80}")
        ...
        for segment in results['temporal_analysis']:
            ...
            print(f"{segment['time_range']:>8} | {sentiment_emoji} Sentimento: {segment['sentiment']:+.2f} | "
                  f"Palavras: {segment['word_count']:>3}")
        ...
        if results['concept_network']:
            ...
            for i, connection in enumerate(results['concept_network'][:10], 1):
                ...
                print(f"{i:>2}. {connection['concept1']:>12} ↔ {connection['concept2']:<12} "
                      f"{strength_bar} ({connection['strength']})")
        ...
        if ling['topic_patterns']:
            ...
            for topic, patterns in ling['topic_patterns'].items():
                print(f"   • {topic[:30]}... | Hesitações: {patterns['hesitations']} | "
                      f"Incerteza: {patterns['uncertainty_markers']}")
        
        print(f"\n{'='*80}")
        print(f"📊 RELATÓRIO CONCLUÍDO")
        print(f"{'='*80}")

def analyze_interview_file(filename, participant_name=None):
    """Função principal para analisar arquivo de transcrição"""
    try:
        ...
        analyzer = QuickInterviewAnalyzer()
        results = analyzer.generate_quick_report(text, participant_name)
...
def main(path=None, participant_name=None):
    print("🚀 ANALISADOR QUICK & DIRTY PARA ENTREVISTAS")
    print("=" * 50)
    ...
    # Executar análise
    resultado = analyze_interview_file(arquivo_transcricao, nome_participante)
    
    if resultado:
        print("\n✅ Análise concluída com sucesso!")
        print("💡 Este é um relatório automático. Valide os insights com análise qualitativa tradicional.")
    else:
        print("\n❌ Falha na análise.")
